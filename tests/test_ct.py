...
trees. These are the C-mirroring low-level operations, exercised here through
the public `falcon_det1024.bindings` module.
"""
...
def ct(sig: bytes) -> bytes:
    return fp.bindings.convert_compressed_to_ct(sig)


def test_ct_shape(ct: bytes) -> None:
    assert len(ct) == fp.CT_SIGNATURE_SIZE == 1538
    assert ct[0] == (0x5A | 0x80)  # det1024 CT header
    assert fp.bindings.get_salt_version(ct) == fp.CURRENT_SALT_VERSION


def test_salt_version_on_both_formats(sig: bytes, ct: bytes) -> None:
    assert fp.bindings.get_salt_version(sig) == fp.bindings.get_salt_version(ct) == 0


def test_ct_verifies(signer: fp.FalconSigner, ct: bytes) -> None:
    # CT verify path accepts the converted signature
    assert fp.bindings.verify_ct(signer.public_key, MSG, ct) is None
...
def test_ct_tamper_rejected(signer: fp.FalconSigner, ct: bytes) -> None:
    ...
    with pytest.raises(fp.InvalidSignature):
        fp.bindings.verify_ct(signer.public_key, MSG, bytes(bad))
...
def test_ct_wrong_length_rejected(signer: fp.FalconSigner, ct: bytes) -> None:
    ...
    with pytest.raises(ValueError):
        fp.bindings.verify_ct(signer.public_key, MSG, ct[:-1])
...
def test_hash_to_point_coeffs_deterministic() -> None:
    a = fp.bindings.hash_to_point_coeffs(MSG)
    b = fp.bindings.hash_to_point_coeffs(MSG)
    assert a == b == fp.bindings.hash_to_point_coeffs(MSG, fp.CURRENT_SALT_VERSION)
    assert len(a) == fp.N
    assert all(0 <= x < 12289 for x in a)
    assert fp.bindings.hash_to_point_coeffs(b"different") != a


def test_s2_and_s1_coeffs_roundtrip(signer: fp.FalconSigner, ct: bytes) -> None:
    h = fp.bindings.pubkey_coeffs(signer.public_key)
    c = fp.bindings.hash_to_point_coeffs(MSG)
    s2 = fp.bindings.s2_coeffs(ct)
    assert len(s2) == fp.N
    # s1 = c - s2*h; succeeds only if (s1, s2) is a valid short vector
    s1 = fp.bindings.s1_coeffs(h, c, s2)
    assert len(s1) == fp.N


def test_convert_rejects_bad_input() -> None:
    with pytest.raises(fp.ConversionError):
        fp.bindings.convert_compressed_to_ct(b"\x00")  # too short
    with pytest.raises(fp.ConversionError):
        fp.bindings.convert_compressed_to_ct(b"\x00" * 100)  # wrong header / undecodable
